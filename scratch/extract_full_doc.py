"""
Run full Gemini extraction on the NCERT PDF (all 107 pages).
Saves final JSON to outputs/ directory.
"""
import sys
import json
import logging
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(ROOT_DIR / "outputs" / "ncert_extraction.log", encoding="utf-8"),
    ]
)
logger = logging.getLogger("full_extract")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"Starting full Gemini extraction: {pdf_path.name}")
    logger.info("=" * 60)

    _, final_json = extract_document_with_gemini(pdf_path, output_dir)
    ...
    print("=" * 60)
    print(f"\nJSON saved to: {output_dir / (pdf_path.stem + '_extracted.json')}")
